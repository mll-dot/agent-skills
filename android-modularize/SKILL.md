...
- 页面分包名：
  ...
  - ui/forgotPwd/    → ForgotPwdActivity, ForgotPwdViewModel
...
**迁移时需要同步处理：**
- **资源文件：先全局搜索判断归属再迁移**
  - **全局搜索 icon/drawable 引用**：如果仅当前业务使用，从 app/res 迁入业务模块 res，确保 `resourcePrefix` 命名
  - 如果被多个业务/模块共用（全局搜索发现多处引用），**下沉到项目公共 UI 组件模块**，不要随业务迁移
  - 业务专属资源（如登录模块的验证码图标、订单模块的专属状态图标）随业务模块迁移，通用资源（如箭头、占位图、通用按钮图标）放公共组件
